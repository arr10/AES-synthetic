...
if __name__ == "__main__":
    ...
    try:    
        df_temp = pd.read_csv(f'data/fold_0/test_ans_fewshot_set{set}.tsv', sep='\t')
    except:
        df_temp = pd.DataFrame(columns = df.columns)
        df_temp.to_csv(f'data/fold_0/test_ans_fewshot_set{set}.tsv', sep='\t', index=False)
    ...
    df_sample = pd.read_csv(f'data/fold_0/sample_set{set}.tsv', delimiter='\t')
    few_shot = []
    for i, row in df_sample.iterrows():
        a, b = get_ruburic(set)
        rubric_names = [rubric_short_names[r] for r in b]
        rubric_scores = [int(row[rubric]) for rubric in rubric_names]
        prompt = create_prompt(row)
        few_shot.append(
            {"role": "user", "content": prompt},
        )
        few_shot.append(
            {"role": "assistant", "content": ', '.join([f"{k}: {score}" for k, score in zip(b, rubric_scores)])}
        )
    ...
    df.to_csv(f'data/fold_0/test_final_ans_fewshot_set{set}.tsv', sep='\t', index=False)
    df.head()